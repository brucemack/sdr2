...
int i2c_put_data(uint8_t addr, const uint8_t *src, size_t len, bool nostop)
{
	int r = i2c_write_blocking(_hw, addr, src, len, nostop);
	sleep_us(I2C_LINGER_US);
	return(r);
}

int i2c_get_data(uint8_t addr, uint8_t *dst, size_t len, bool nostop)
{
	int r = i2c_read_blocking(_hw, addr, dst, len, nostop);
	sleep_us(I2C_LINGER_US);
	return(r);
}
// --------------------------------------------------------------------------
...
void si_enable(int i, bool en)
{
	...
	i2c_put_data(I2C_VFO, &data[0], 1, true);
	// TODO: FIGURE OUT WHY THIS ISN'T USING THE STANDARD FUNCTION
	i2c_read_blocking(_hw, I2C_VFO, &data[1], 1, false);
	...
	i2c_put_data(I2C_VFO, &data[0], 2, false);
}
...
int si_getreg(uint8_t *data, uint8_t reg, uint8_t len)
{
	...
	ret = i2c_put_data(I2C_VFO, &reg, 1, true);
	if (ret<0) printf ("I2C write error\n");
	ret = i2c_get_data(I2C_VFO, data, len, false);
...
void si_setmsn(int i)
{
	...
	i2c_put_data(I2C_VFO, data, 9, false);
}
...
void si_setmsi(int i)
{
	...
	if (i==0)
	{
		...
		if ((vfo[0].phase==PH180)||(vfo[0].phase==PH270))					// Phase is 180 or 270 deg?
		{
			...
			i2c_put_data(I2C_VFO, data, 3, false);
		}
		else
		{
			data[0] = SI_CLK0_CTL;											// set the invert flag
...
void si_init(i2c_inst_t* hw) {
	...
	data[2] = SI_VFO0CTL;
	data[3] = SI_VFO1CTL;
	i2c_put_data(I2C_VFO, data, 4, false);
